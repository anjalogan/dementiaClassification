...
def make_data_loaders(batch_size, num_workers: int = 2):
    ...
    for i in range(len(data)):
        row = np.expand_dims(np.expand_dims(data[i, 1:cols-1], 0), 0)
        val_data.append((torch.from_numpy(row),torch.tensor((pd.to_numeric(data[i, cols-1],downcast="integer")), dtype=torch.int8)))
    ...
    for i in range(len(data)):
        row = np.expand_dims(np.expand_dims(data[i, 1:cols-1], 0), 0)
        train_data.append((torch.from_numpy(row),torch.tensor((pd.to_numeric(data[i, cols-1],downcast="integer")), dtype=torch.int8)))
    ...
    for i in range(len(data)):
        row = np.expand_dims(np.expand_dims(data[i, 1:cols-1], 0), 0)
        test_data.append((torch.from_numpy(row),torch.tensor((pd.to_numeric(data[i, cols-1],downcast="integer")), dtype=torch.int8)))
...
def load_data():
    save_csv(list_data("validate"), 'validate.csv')
    save_csv(list_data("train"), 'train.csv')
    save_csv(list_data("test"), 'test.csv')


def main():
    #load_data()
    train_dataloader, valid_dataloader, test_dataloader = make_data_loaders(batch_size=1)
    ...
    model = models.get(Models.RESNET18, num_classes=4, num_input_channels=1)
    training_params = {
        "max_epochs": 1,
        "initial_lr": 0.01,
        "loss": "cross_entropy",
